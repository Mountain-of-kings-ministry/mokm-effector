...
unsigned int TriangleNode::nPorts(QtNodes::PortType portType) const
{
    switch (portType) {
...
QtNodes::NodeDataType TriangleNode::dataType(QtNodes::PortType portType, QtNodes::PortIndex portIndex) const
{
    Q_UNUSED(portType)
    Q_UNUSED(portIndex)
    return { "layer", "Layer" };
...
void TriangleNode::setInData(std::shared_ptr<QtNodes::NodeData> nodeData, QtNodes::PortIndex portIndex)
{
    Q_UNUSED(nodeData)
    Q_UNUSED(portIndex)
}

std::shared_ptr<QtNodes::NodeData> TriangleNode::outData(QtNodes::PortIndex port)
{
    Q_UNUSED(port)
    return m_output;
}

QWidget *TriangleNode::embeddedWidget()
{
...
QtNodes::NodeValidationState TriangleNode::validationState() const
{
    return m_validationState;
}

QJsonObject TriangleNode::save() const
{
    QJsonObject obj;
    obj["width"] = m_width;
    obj["height"] = m_height;
...
void TriangleNode::load(QJsonObject const &obj)
{
...
void TriangleNode::generateOutput()
{
    if (m_output) {
    ...
    shape->setShapeType(ShapeLayer::Triangle);
    shape->setShapeWidth(m_width);
    shape->setShapeHeight(m_height);
    shape->setColor(m_color);
    shape->setName("Triangle Output");
