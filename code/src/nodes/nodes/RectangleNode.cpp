...
unsigned int RectangleNode::nPorts(QtNodes::PortType portType) const
{
    switch (portType) {
    case QtNodes::PortType::In:  return 0; // Generator - no inputs
    case QtNodes::PortType::Out: return 1; // One visual output
    default: return 0;
    }
}

QtNodes::NodeDataType RectangleNode::dataType(QtNodes::PortType portType, QtNodes::PortIndex portIndex) const
{
    Q_UNUSED(portType)
    Q_UNUSED(portIndex)
    return { "layer", "Layer" };
...
void RectangleNode::setInData(std::shared_ptr<QtNodes::NodeData> nodeData, QtNodes::PortIndex portIndex)
{
    Q_UNUSED(nodeData)
    Q_UNUSED(portIndex)
    // Generator - no inputs to process
}

std::shared_ptr<QtNodes::NodeData> RectangleNode::outData(QtNodes::PortIndex port)
{
    Q_UNUSED(port)
    return m_output;
}

QWidget *RectangleNode::embeddedWidget()
{
...
QtNodes::NodeValidationState RectangleNode::validationState() const
{
    return m_validationState;
}

QJsonObject RectangleNode::save() const
{
    QJsonObject obj;
    obj["width"] = m_width;
    obj["height"] = m_height;
...
void RectangleNode::load(QJsonObject const &obj)
{
...
void RectangleNode::generateOutput()
{
    // Delete old output layer before creating new one
    if (m_output) {
    ...
    // Create a ShapeLayer as the output
    auto *shape = new ShapeLayer();
    shape->setShapeType(ShapeLayer::Rectangle);
    shape->setShapeWidth(m_width);
    shape->setShapeHeight(m_height);
    shape->setColor(m_color);
    shape->setName("Rectangle Output");
